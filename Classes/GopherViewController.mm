...
- (UILabel *) makeScoreLabel
{
	...
	carrot.image = [UIImage imageNamed:@"Carrot.png"];
	...
	if(currentHigh < currentScore)
	{
		...
		NSString *scoreString = [NSString stringWithFormat:formatString, 0, 0 *10, 0, 0*20, currentScore];
...
- (void)initStuff 
{
	...
	gopherView.animationInterval = 1.0 / 30.0;
	...
	
	{
		...
		[ self updateScore: score withDead:0 andTotal: 10];
